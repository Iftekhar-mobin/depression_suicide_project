...
%\author*[1]{\fnm{Md Iftekharul} \sur{Mobin}}\email{iftekhar.mobin@aiub.edu}
%
%\author[2]{\fnm{S.M. Hasan} \sur{Mahmud}}\email{hasan.swe@aiub.edu}
%%\equalcont{These authors contributed equally to this work.}
%
%\author[3]{\fnm{Debajyoti} \sur{Karmaker}}\email{d.karmaker@aiub.edu}
%%\equalcont{These authors contributed equally to this work.}
%
%\affil[1,2,3]{\orgdiv{Computer Science and Engineering}, \orgname{American International University-Bangladesh(AIUB)}, \orgaddress{\street{408/1, Kuratoli}, \city{Dhaka}, \postcode{1229}, \state{Dhaka}, \country{Bangladesh}}}
...
\begin{itemize}
\item Background literature is studied in section \ref{lit_review} which contains following subsections 
\begin{enumerate} [label=(\roman*)]
\item Subsection \ref{Text_content_lda} illustrates LDA for Textbook related articles 
\item Subsection \ref{dom_lda} explains LDA for dominant keywords determination
\item Subsection \ref{sent_lda} describes LDA extracted keywords can be used to realize sentiment
\end{enumerate}
\item Section \ref{methods} methodology segregated into three parts
\begin{enumerate} [label=(\roman*)]
\item Prototype dataset, Data pre-processing technique, models (subsection \ref{data_pre})
\item Topic modeling, LDA, comparative analysis and coherence determination (subsection \ref{top_mod_feature}) 
\item Survey response visualization and analysis methods LIWC, Word Cloud (subsection \ref{vis_part})
\end{enumerate}
\item Next section \ref{exp_analysis} demonstrated the experiments and contains analysis separated into several subsections. 
\begin{enumerate} [label=(\roman*)]
\item Coherence measurements (subsection \ref{coh_measure})
\item Dominant keywords and topic extraction (subsection \ref{dom_topic}) 
\item Topics visualization and most frequent terms exploration (subsection \ref{top_term_vis})
\item Survey results and analysis (subsection \ref{qual_survey})
\end{enumerate}
\item Then results discussion, limitations and conclusion are illustrated in section \ref{res_dis}, \ref{limitat} and \ref{conclu} respectively. 
\end{itemize}
