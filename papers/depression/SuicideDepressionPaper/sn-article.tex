...
Various machine learning classification algorithms are used mentioned in the Table \ref{Table_classification_alg}. The hyper-parameters are mostly sklearns' default settings. Two step of Gridsearch technique of sklearn library is used. From the selected classifiers to determine the best classifiers default parameters are applied and SVM showed high accuracy. Then, to achieve highest accuracy hyper-parameters are feed into grid search. Using various set of parameters, from the result and experiments we found almost 60\% accuracy for SVM model to predict the suicide intensity categories. From various set of values applying gridsearch for SVM SVC we found degree=2, gamma=0.7, kernel=rbf showed the highest accuracy. 
%
\begin{table}[h]
\begin{center}
\begin{flushleft}
