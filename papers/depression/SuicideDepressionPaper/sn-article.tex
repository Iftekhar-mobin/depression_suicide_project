...
LDA is a versatile tool to investigate various keywords in subtle topic within context. LDA driven results of various categories are visualized in this study to observe the latent topic and terms using pyldvis library in figure~\ref{indicator_pyldvis}, \ref{Ideation_pyldvis}, \ref{Behavior_pyldvis} and \ref{Attempt_pyldvis}. 
...
\end{figure}
In Test dataset samples a large number of documents are having low features. In figure~\ref{fig_Test_Dataset_token_frequency} the frequency of tokens in each class is depicted. 
%
\begin{figure}[H]
...
\end{figure}
%
From figure~\ref{fig_Test_Dataset_token_frequency} and figure~\ref{features_vis_pro_exp} the following comments can be inferred
\begin{enumerate}
...
\end{enumerate}
However, it does not reveals any direct clues in terms of hypothetical relationships between the two category. It is difficult find pattern in which we can determine the depression and suicidal thought.  
